RSpec.describe Bunto::SeoTag do
  let(:page)      { make_page }
  let(:site)      { make_site }
  let(:post)      { make_post }
  ...
  it "builds" do
    expect(output).to match(%r!Bunto SEO tag!i)
  end

  it "outputs the plugin version" do
    version = Bunto::SeoTag::VERSION
    expect(output).to match(%r!Bunto SEO tag v#{version}!i)
  end
...
EOS
  ...
  context "facebook" do
    let(:site_facebook) do
      {
        "admins"    => "buntowaf-fb-admins",
        "app_id"    => "buntowaf-fb-app_id",
        "publisher" => "buntowaf-fb-publisher",
      }
    end
    ...
    it "outputs facebook admins meta" do
      expected = %r!<meta property="fb:admins" content="buntowaf-fb-admins" />!
      expect(output).to match(expected)
    end

    it "outputs facebook app ID meta" do
      expected = %r!<meta property="fb:app_id" content="buntowaf-fb-app_id" />!
      expect(output).to match(expected)
    end

    it "outputs facebook article publisher meta" do
      expected = %r!<meta property="article:publisher" content="buntowaf-fb-publisher" />!
      expect(output).to match(expected)
    end
  end
  ...
  context "twitter" do
    context "with site.twitter.username" do
      let(:site_twitter) { { "username" => "buntowaf" } }
      ...
      context "with page.author as a string" do
        ...
        it "outputs twitter card meta" do
          ...
          expected = %r!<meta name="twitter:site" content="@buntowaf" />!
          expect(output).to match(expected)
